...
class PwParser(BaseParser):
    ...
    def parse(self, **kwargs):
        ...
        if os.path.exists(self.directory / crash_file_filename):
            with open(self.directory / crash_file_filename, "r") as f:
                crash_file = f.read()

        parameters = self.parameters
        parsed_xml, logs_xml = self.parse_xml(dir_with_bands, parser_options)
        parsed_stdout, logs_stdout = self.parse_stdout(
            parameters, parser_options, parsed_xml, crash_file
        )
    ...
    def get_calculation_type(self):
        ...
        return (
            self.parameters["input_data"].get("CONTROL", {}).get("calculation", "scf")
        )
    ...
    def validate_premature_exit(self, logs):
        ...
        if (
            "ERROR_OUT_OF_WALLTIME" in logs["error"]
            and "ERROR_OUTPUT_STDOUT_INCOMPLETE" in logs["error"]
        ):
    ...
    def validate_ionic(self, trajectory, parameters, logs):
        ...
        electronic_convergence_reached = (
            "ERROR_ELECTRONIC_CONVERGENCE_NOT_REACHED" not in logs.error
        )
        ionic_convergence_reached = (
            "ERROR_IONIC_CONVERGENCE_NOT_REACHED" not in logs.error
        )
        bfgs_history_failure = "ERROR_IONIC_CYCLE_BFGS_HISTORY_FAILURE" in logs.error
        maximum_ionic_steps_reached = (
            "ERROR_MAXIMUM_IONIC_STEPS_REACHED" in logs.warning
        )
        ...
        if bfgs_history_failure:
            ...
            if not electronic_convergence_reached:
                return (
                    self.exit_codes.ERROR_IONIC_CYCLE_BFGS_HISTORY_AND_FINAL_SCF_FAILURE
                )
    ...
    def is_ionically_converged(self, trajectory, except_final_scf=False):
        ...
        from aiida_quantumespresso.utils.validation.trajectory import (
            verify_convergence_trajectory,
        )

        relax_type = self.get_calculation_type()
        parameters = self.parameters["input_data"]
        threshold_forces = parameters.get("CONTROL", {}).get(
            "forc_conv_thr", pw.forc_conv_thr
        )
        threshold_stress = parameters.get("CELL", {}).get(
            "press_conv_thr", pw.press_conv_thr
        )
    ...
    def parse_xml(self, dir_with_bands=None, parser_options=None):
        ...
        xml_files = ["data-file-schema.xml"]
        ...
        try:
            ...
            self.exit_code_xml = (
                self.exit_codes.ERROR_UNEXPECTED_PARSER_EXCEPTION.format(exception=exc)
            )

        return parsed_data, logs

    def parse_stdout(
        self, parameters, parser_options=None, parsed_xml=None, crash_file=None
    ):
        ...
        if not os.path.exists(self.directory / self.outputname):
            self.exit_code_stdout = self.exit_codes.ERROR_OUTPUT_STDOUT_MISSING
            return parsed_data, logs

        try:
            with open(self.directory / self.outputname) as f:
                stdout = f.read()
        except IOError:
        ...
        try:
            parsed_data, logs = parse_stdout(
                stdout, parameters, parser_options, parsed_xml, crash_file
            )
        except Exception as exc:
            logs.critical.append(traceback.format_exc())
            self.exit_code_stdout = (
                self.exit_codes.ERROR_UNEXPECTED_PARSER_EXCEPTION.format(exception=exc)
            )
        ...
        if "atoms" not in parsed_data["structure"]["cell"]:
            # symbols = {atom.symbol: atom.symbol for atom in structure}
            parsed_data["structure"]["cell"]["atoms"] = [
                (atom.symbol, atom.position) for atom in structure
            ]
    ...
    def build_output_structure(self, parsed_structure):
        ...
        if type_calc not in ["relax", "vc-relax", "md", "vc-md"] or "cell" not in list(
            parsed_structure.keys()
        ):
            return self.atoms
    ...
    def build_output_trajectory(parsed_trajectory, structure):
        ...
        if "atomic_positions_relax" in parsed_trajectory:
            ...
            positions = numpy.array([[site.position for site in structure]])
        ...
        symbols = [str(site.symbol) for site in structure]
    ...
    def build_output_bands(self, parsed_bands, parsed_kpoints=None):
        ...
        if len(parsed_bands["occupations"]) > 1:
            ...
            occupations = 2.0 * numpy.array(parsed_bands["occupations"][0])
        ...
        # bands = orm.BandsData()
        bands = {}
        # bands["kpointsdata"] = parsed_kpoints
        bands["bands"] = {
            "energies": bands_energies,
    ...
    def get_extended_symmetries(self):
        ...
        parameters = self.results["output_parameters"]
        ...
        for element in symmetries_reduced:
            ...
            symmetry["rotation"] = possible_symmetries[element["symmetry_number"]][
                "matrix"
            ]
            symmetry["inversion"] = possible_symmetries[element["symmetry_number"]][
                "inversion"
            ]
